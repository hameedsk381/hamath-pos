/**
 * Maatlaadi Bill - Intelligent Telugu & English Product & Customer Matcher
...
window.TELUGU_NUMBER_MAP = TELUGU_NUMBER_MAP;
window.UNIT_SYNONYMS = UNIT_SYNONYMS;
...
class ProductMatcher {
  ...
  /**
   * Normalize Telugu / English units
   */
  ...
  /**
   * Parse spoken quantity into numerical value
   */
  ...
  /**
   * Match spoken item against product catalogue
   * @param {string} spokenName
   * @returns {Object} { status: 'exact'|'ambiguous'|'unknown', product, candidates, confidence }
   */
  matchProduct(spokenName) {
    ...
    for (const p of products) {
      if (p.name.toLowerCase() === cleanSpoken || p.name_te.toLowerCase() === cleanSpoken) {
        return { status: 'exact', product: p, candidates: [p], confidence: 1.0 };
      }
      if (p.aliases && p.aliases.some(alias => alias.toLowerCase() === cleanSpoken)) {
        // Special case: generic aliases like "cement" or "paint" may match multiple brands!
    ...
    // 2. Multi-match ambiguity check for broad categories (e.g. "cement", "సిమెంట్", "పెయింట్", "table", "chair")
    ...
    for (const token of broadTokens) {
      if (cleanSpoken === token || cleanSpoken.startsWith(token + ' ') || cleanSpoken.endsWith(' ' + token)) {
        const matches = products.filter(p =>
          ...
          p.name_te.toLowerCase().includes(token) ||
    ...
    // If top match is decisively better (> 0.82 and > 0.15 higher than second)
    if (
    ...
    // Otherwise, ambiguous: present top 3 options
    return {
      status: 'ambiguous',
      product: null,
  ...
  /**
   * Match customer name from spoken text
   * Handles Telugu suffixes like 'కి', 'గారికి', 'నుండి' (e.g. "రమేష్కి" -> "రమేష్")
   */
  matchCustomer(spokenCustomer) {
    if (!spokenCustomer) return null;

    let clean = spokenCustomer.trim();
    // Strip common Telugu case suffixes
...
window.ProductMatcher = new ProductMatcher();
