/**
 * Maatlaadi Bill - Local Data Store & Mock Auth Layer
 * Zero-external-dependency robust persistent storage with LocalStorage
 */
...
class DataStore {
  ...
  init() {
    ...
    // First, process all seed products (ensuring fresh aliases and Telugu names)
    SEED_PRODUCTS.forEach(sp => {
      const existing = existingMap.get(sp.id);
      if (existing) {
        // Merge aliases so user custom aliases aren't lost, but new seed aliases are included
    ...
    // Append any user-created custom products (sanitizing any corrupt glyphs)
    ...
    // 7. Initialize Settings (Production mode by default)
  ...
  getBusiness() {
    return this.get(STORAGE_KEYS.BUSINESS);
  }
  ...
  convertQuotationToInvoice(quotationId) {
    ...
    const invoice = {
      ...
      items: quotation.items.map(item => ({ ...item, id: 'ini-' + Date.now() + Math.random().toString(36).substr(2, 4) }))
    };
